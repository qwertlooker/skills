...
The practical default is informed by the public Jiaopian ICT Talent Ecosystem mainslides: light canvas, full red takeaway titles, gray structural lines, substantial diagrams or evidence, compact footers, and red attached to meaningful content groups. Do not copy its logo, security footer, internal identifiers, QR codes, customer-specific material, or measured outcomes.

## Choose one title system

Use one system consistently across a deck unless an inherited template specifies another:

1. **ICT business mainslide, default:** set the complete slide title in Jiaopian red; use no top strip and no title underline. Keep the remaining page mostly dark gray and neutral. The full red title does not consume the page's one supporting red content group.
...
- Do not add a short red locator line above every title merely to signal the style.
...
- Use `#C7000B` as the default Jiaopian red; use `#990000` only to match an inherited legacy template.
